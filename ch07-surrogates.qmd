...
#| eval: true
#| echo: false
# Representative output for a single d=2 replicate (cached from Hopper runs)
print("RMSE: 2.168, CRPS: 1.224, Coverage: 0.894")
```
...
@fig-jump-comparison shows the 1D cross-section more clearly: the GP's predicted mean (red) overshoots on one side and undershoots on the other due to the Gibbs-like smoothing artifact, while the IQN (blue) transitions sharply at the boundary.

```{python}
#| eval: true
#| echo: false
#| label: fig-jump-comparison
#| fig-cap: "1D jump comparison: the stationary GP (red) blurs across the jump boundary, producing oscillation artifacts on both sides. The IQN (blue) captures the sharp transition cleanly, with uncertainty bands that widen only at the boundary."
from IPython.display import Image
display(Image("cache/fig-jump-comparison.png"))
```
...
Using 50 random 80:20 train/test splits (@tbl-moto), GBC matches hetGP on CRPS within standard-error overlap. The GP achieves better coverage because its explicit noise model $\sigma^2(x)$ provides well-calibrated Gaussian intervals when the heteroscedastic Gaussian assumption holds. Ensembling closes part of the coverage gap by pooling quantile samples, widening intervals in regions of model disagreement.

The take-away is that GBC, with no custom noise model, is competitive with a specialist GP even on its home turf, a small ($n = 133$), one-dimensional, heteroscedastic but Gaussian-conditional problem. The IQN's flexibility is not a liability even when the GP's assumptions are met.
...
These benchmark results hold in moderate-SNR settings. On high-SNR problems the multiplicative τ-architecture can under-disperse, producing calibration failures; see @sec-calibration for the diagnostic and remedy.
...
#| eval: true
#| echo: false
# Representative output for a single Friedman replicate (cached from Hopper runs)
print("RMSE: 0.520")
print("CRPS: 0.323")
print("Coverage: 0.991")
```
...
The scaling patterns in @tbl-scaling illustrate @prp-scaling. From $n = 500$ to $n = 1{,}000$ (a $2\times$ increase), GP time grows from 2.9s to 9.7s, a factor of roughly $3.3\times$, between the $\mathcal{O}(n^2)$ prediction of $4\times$ and the $\mathcal{O}(n^3)$ prediction of $8\times$. From $n = 1{,}000$ to $n = 2{,}000$ (another $2\times$), time grows from 9.7s to 47.6s, a factor of $4.9\times$, trending toward the cubic rate. At these modest sample sizes, the $\mathcal{O}(n^2)$ gradient computation for MLE contributes alongside the $\mathcal{O}(n^3)$ Cholesky factorization; the true cubic behavior dominates at larger $n$. Extrapolating at the asymptotic $\mathcal{O}(n^3)$ rate, $n = 5{,}000$ would require approximately $47.6 \times (5000/2000)^3 \approx 743$ seconds for a single MLE evaluation, and the optimizer may require dozens of iterations. The super-quadratic growth makes the dense GP impractical beyond $n = 2{,}000$ on standard hardware.
...
#| eval: true
#| echo: false
# Cached GBC training times from Hopper (NVIDIA A100)
print("n =    500:   9.6s")
print("n =   1000:  10.4s")
print("n =   2000:  17.7s")
print("n =   5000:  41.2s")
print("n =  10000:  70.6s")
print("n =  20000: 152.5s")
```
...
Conversely, GPU-accelerated GP implementations (e.g., GPyTorch with CUDA) can push the feasibility boundary somewhat by leveraging GPU parallelism for the Cholesky decomposition and kernel evaluations. With GPyTorch, dense GP inference at $n = 10{,}000$ becomes feasible (though slow), and sparse variational GPs with conjugate gradients can scale further. But the cubic asymptotic cost of the dense GP remains, and GPU acceleration does not change the exponent, it only reduces the constant. At $n = 90{,}000$, even GPU-accelerated dense GPs are infeasible, while GBC trains comfortably.
:::

@fig-scaling-comparison visualizes the scaling behavior of the two methods. The GP training time grows cubically (the curve steepens on a log scale), while GBC grows linearly. The crossover occurs near $n = 1{,}500$.

```{python}
#| eval: true
#| echo: false
#| label: fig-scaling-comparison
#| fig-cap: "Scaling comparison: GP training time grows as $O(n^3)$ while GBC grows as $O(n)$. Beyond $n \\approx 2{,}000$ the GP is computationally infeasible (dashes). GBC continues to improve with additional data, reaching RMSE = 0.29 at $n = 20{,}000$."
from IPython.display import Image
display(Image("cache/fig-scaling-comparison.png"))
```


## Satellite Drag: GBC on a Smooth High-Dimensional Problem {#sec-surr-satellite}

The benchmarks above tested non-stationary and interaction-heavy responses where GBC has structural advantages over GPs. Can GBC also compete in the GP's area of strength: smooth response surfaces in moderate dimensions? The satellite drag problem (@sec-data-satellite) provides this complementary test.

The Los Alamos National Laboratory Test Particle Monte Carlo (TPMC) simulator [@mehta2014modeling] predicts the drag coefficient of satellites in low Earth orbit as a function of seven thermospheric input parameters: velocity, surface temperature, atmospheric temperature, yaw, pitch, normal energy accommodation coefficient, and tangential momentum accommodation coefficient. The response surface is smooth with no regime transitions or jump boundaries. This is precisely the setting where GPs excel, and where GBC must demonstrate it can at least match their performance.

The practical stakes are high. On February 3, 2022, SpaceX launched 49 Starlink satellites into low Earth orbit. A geomagnetic storm elevated thermospheric density; within days, 38 of the 49 satellites reentered the atmosphere, an estimated \$100 million loss [@Berger23]. Accurate drag-coefficient prediction with honest uncertainty quantification could have informed the decision to delay launch. As the number of satellites in low Earth orbit grows (SpaceX alone plans over 40,000 Starlink satellites), fast and uncertainty-aware drag prediction becomes operationally critical.

We use the dataset of one million simulation runs for the Hubble Space Telescope geometry provided by @sun2019emulating, splitting 20\% for training and 80\% for evaluation. The IQN uses three hidden layers of width 256, cosine embedding with $M = 64$ frequency components, Adam optimizer, batch size 2048, and 200 training epochs. Training on 200,000 data points takes about 10 minutes on a single GPU. @sauer2023non surveys GP-based models on this same dataset: their best (treed-GP) achieves RMSE = 0.08 and CRPS = 0.04; their worst (deep GP) gives RMSE = 0.23. Our IQN achieves RMSE = 0.098 and CRPS = 0.05 ,  competitive with the best GP at a fraction of the out-of-sample prediction cost.

```{python}
#| eval: false
#| echo: false
#| label: fig-satellite-yhat
#| fig-cap: "Satellite drag coefficient: GBC predicted vs. observed values on 800,000 held-out test points. RMSE = 0.098 and CRPS = 0.05, competitive with the best GP models."
from IPython.display import Image
display(Image("cache/fig-satellite-yhat.png"))
```

The predicted-vs-observed scatter hugs the diagonal and the residuals are approximately Gaussian. The 95% prediction intervals capture the true drag coefficient reliably, with interval width reflecting genuine uncertainty about the gas-surface interaction physics. On a smooth problem that suits GPs well, GBC is a viable alternative ,  not a clear winner, but fully competitive. The real advantage of GBC emerges on the non-stationary and jump problems above; this benchmark establishes that GBC does not sacrifice smooth-function accuracy in exchange for that flexibility.
