...
With the kernel specified, the GP provides closed-form predictions that are the envy of the Bayesian modeling world. No sampling, no MCMC, no variational approximation --- just linear algebra.
...
The beauty of @eq-gp-mean and @eq-gp-var is that they are exact --- no sampling, no approximation, no iterative algorithm. The computational cost is dominated by the matrix inversion (or equivalently, Cholesky decomposition), which we discuss in @sec-surr-gp-limits.
...
The cubic cost of GP inference has generated a cottage industry of approximation methods, each trading fidelity for scalability. These methods are important to understand because a skeptical reader might reasonably ask: "Why not just fix the GP's scalability problem and keep its other advantages?" The answer is that scalability fixes exist but each introduces new assumptions, new tuning parameters, and new failure modes --- and none addresses the stationarity or Gaussian-conditional limitations. Before introducing GBC as an alternative, it is worth understanding what these approximations buy and what they give up.
...
It is worth noting that local GPs have a structural affinity with the IQN in one respect: both methods make predictions using only a "local" subset of information. The IQN achieves this through its learned representation --- the hidden layers project the input into a feature space where only nearby patterns in feature space (not necessarily nearby in input space) influence the prediction. The difference is that the IQN's "neighborhoods" are learned end-to-end rather than defined by Euclidean distance, giving it more flexibility to handle non-stationary structure.
...
The conceptual shift from GP to GBC is significant. The GP models the response as a random function drawn from a Gaussian prior and conditions on observed data to obtain a Gaussian posterior. Everything is Gaussian, everything is analytic, and the kernel encodes all prior knowledge.
...
In this section we articulate the case for GBC as a surrogate, being careful to present both advantages and limitations. The comparison between GP and GBC surrogates is best organized around three fundamental themes: computational scaling, distributional flexibility, and the small-sample tradeoff. Understanding all three is necessary for making an informed choice in practice.

### Computational Scaling: $\mathcal{O}(n)$ versus $\mathcal{O}(n^3)$ {#sec-surr-scaling-theory}

The most consequential difference is computational, and it is worth being precise about where the costs arise. The GP's dominant cost is the Cholesky decomposition of the $n \times n$ covariance matrix $\mathbf{K} + \sigma^2_\varepsilon \mathbf{I}$, which requires $n^3/3$ floating-point operations and $n^2/2$ storage. This decomposition must be performed for every evaluation of the log marginal likelihood (@eq-gp-mll) during hyperparameter optimization, and typical optimizers require 10--100 such evaluations. The total training cost is therefore $\mathcal{O}(n^3)$ with a constant factor proportional to the number of optimizer iterations.

The IQN is trained by mini-batch SGD, with cost $\mathcal{O}(n \cdot E \cdot L \cdot w^2)$ per epoch, where $E$ is the number of epochs, $L$ the number of layers, and $w$ the layer width. Crucially, this is linear in $n$. There is no covariance matrix to invert. The per-batch cost is $\mathcal{O}(B_{\text{batch}} \cdot L \cdot w^2)$ where $B_{\text{batch}}$ is the mini-batch size, and the number of batches per epoch is $n / B_{\text{batch}}$. On a modern GPU, training on $n = 100{,}000$ points takes minutes, not the days or weeks that a dense GP would require (if it were feasible at all).
...
Before turning to the multi-dimensional benchmarks, we fit a GBC surrogate on a simple one-dimensional function to build intuition for what the IQN learns and how its predictions differ from a GP's. This pedagogical example trains in seconds on a laptop CPU, and we encourage the reader to reproduce it and experiment with different functions and noise structures.
...
The BGP benchmarks tested non-stationarity --- the setting where GPs are structurally disadvantaged. A skeptic might argue that GBC's advantage on BGP is unsurprising: of course a flexible neural network outperforms a stationary GP on a non-stationary surface. The more interesting question is whether GBC can compete with GPs in *their* area of strength: smooth functions with moderate noise. We now turn to exactly that challenge, using a function that also tests variable selection and what happens when the sample size grows beyond the GP's computational limits.

The Friedman function [@friedman1991multivariate] is a standard benchmark for high-dimensional regression. It was originally introduced to evaluate adaptive regression procedures that can discover relevant inputs from a pool of many candidates. In the computer experiments literature, it serves as a testbed for surrogate methods because it combines smooth but nonlinear structure with a variable-selection challenge. The function is:
$$
...
The Friedman function is straightforward to implement.
...
The most dramatic advantage of GBC over GPs is scalability. The previous two sections compared accuracy at fixed sample sizes; this section examines what happens when we vary $n$ and push beyond the GP's computational reach. A dense GP requires $\mathcal{O}(n^3)$ operations for the Cholesky decomposition, which becomes prohibitive for $n > 5{,}000$. The IQN trains via SGD with cost linear in $n$. This section quantifies the difference using the Friedman function at sample sizes from $n = 500$ to $n = 20{,}000$.

The question is not merely "which method is faster?" --- that is obvious from the complexity analysis. The more interesting question is "how does accuracy scale with $n$?" If GBC's accuracy improves rapidly with additional data, then the ability to train on larger datasets is not just a computational convenience but a path to genuinely better predictions. In the era of cheap simulation (where a single CPU can generate thousands of simulator evaluations overnight), the surrogate that can exploit all available data has a compounding advantage.
...
Before presenting the results, we formalize the scaling comparison. The following proposition is a straightforward consequence of the computational structure of each method, but stating it explicitly clarifies what we expect to see in the empirical results and provides a framework for interpreting the timings.
...
Nonetheless, the improvement is monotonic and does not saturate at the sample sizes we tested, suggesting that even larger datasets (which are available for many modern simulators) would yield further accuracy gains. The GP, by contrast, cannot participate in this conversation: it is limited to whatever accuracy $n \leq 2{,}000$ can provide.
...
We have now seen GBC and GP surrogates compared on smooth functions (Friedman), piecewise-continuous functions (BGP), heteroscedastic data (motorcycle), and across a wide range of sample sizes (scaling study). The results, combined with the motorcycle result in @tbl-moto, paint a consistent picture whose practical implications can be distilled into a decision framework. The choice between GP and GBC surrogates is not a matter of one method dominating the other; rather, it depends predictably on three problem characteristics: the sample size $n$, the smoothness of the response surface $f$, and the nature of the predictive distribution that the application requires. We organize the discussion around these three axes.
