...
@fig-satellite-yhat shows the out-of-sample predicted vs. observed scatter plot. The results are consistent with expectations. The error histogram (the distribution of $\hat{y} - y$ for held-out observations) is approximately Gaussian and centered at zero, with some mild left-tail heaviness indicating occasional underestimates. The scatter plot of predicted versus observed drag coefficients hugs the diagonal, with the model being most accurate for smaller drag coefficient values and slightly less precise at the extremes.

```{python}
#| eval: true
#| echo: false
#| label: fig-satellite-yhat
#| fig-cap: "Satellite drag coefficient: GBC predicted vs. observed values on 800,000 held-out test points. The predictions closely track the diagonal, with RMSE = 0.098 and CRPS = 0.05, competitive with the best GP models at a fraction of the computational cost."
from IPython.display import Image
display(Image("cache/fig-satellite-yhat.png"))
```
