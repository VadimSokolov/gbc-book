...
The heart of the chapter is a sequence of increasingly challenging examples. The conjugate normal-normal model (@sec-bi-normal) validates GBC against an exact posterior and reveals the Wang distortion interpretation. Beta-Binomial and Gamma-Poisson conjugates (@sec-bi-conjugates) confirm accuracy on asymmetric and right-skewed posteriors. The bimodal posterior example (@sec-bi-bimodal) tests GBC on a problem that trips up MCMC and variational methods. The multivariate parameter discussion (@sec-bi-multivariate) extends GBC to the $d > 1$ case, comparing independent and autoregressive strategies. Neal's funnel (@sec-bi-funnel) demonstrates GBC on a hierarchical model that is pathological for HMC. The horseshoe prior (@sec-bi-horseshoe) shows how GBC handles global-local shrinkage posteriors where MCMC diverges. The g-and-k distribution (@sec-bi-gnk), defined by its quantile function with no closed-form density, is the canonical intractable-likelihood benchmark for simulation-based inference. Two applied examples, simulator-based inference for an Ebola agent-based model (@sec-bi-ebola) and satellite drag estimation (@sec-bi-satellite), demonstrate GBC on real scientific problems. Throughout, the amortization advantage (@sec-bi-amortized) emerges as a recurring theme. We conclude by positioning GBC relative to the classical Bayesian toolbox (@sec-bi-vs-classical) and honestly assessing when MCMC remains the better choice (@sec-bi-when-mcmc).
...
@fig-abc-comparison visualizes this contrast on a concrete example: GBC produces sharp posterior estimates from the same simulation budget where ABC must either accept very few samples (small $\varepsilon$) or accept a biased posterior (large $\varepsilon$).

```{python}
#| eval: true
#| echo: false
#| label: fig-abc-comparison
#| fig-cap: "ABC vs. GBC posterior approximations. ABC (left panels) at decreasing tolerance $\\varepsilon$ requires exponentially more simulations for diminishing bias reduction. GBC (right) learns the posterior quantile function from the same simulated dataset, producing sharp posterior samples via a single forward pass."
from IPython.display import Image
display(Image("cache/fig-abc-comparison.png"))
```
...
*Known failure mode.* The multiplicative τ-architecture in Step 3 can under-disperse on high signal-to-noise problems, producing prediction intervals that are too narrow. @sec-calibration documents when this occurs, provides the SNR diagnostic that predicts it in advance, and develops conformal post-processing as a remedy.
...
for epoch in range(n_epochs):
    ...
    x_batch = X_norm[idx]
    y_batch = Y_norm[idx]
    tau = torch.rand(batch_size, 1)  # one tau per observation

    q_hat = model(x_batch, tau)
    residual = y_batch - q_hat
    loss = torch.mean(torch.maximum(tau * residual, (tau - 1) * residual))
...
Sampling a distinct $\tau$ per observation in the batch gives an unbiased Monte Carlo estimate of $\mathbb{E}_\tau[\rho_\tau(\theta - G_\phi(\tau, z))]$ at every gradient step, integrating across quantile levels simultaneously. Using a single shared $\tau$ per batch collapses this integral to a single quantile level and produces high-variance gradient estimates. The training takes about 30 seconds on a modern laptop with no GPU. The final loss value gives a rough indication of the approximation quality: lower is better, and values near 0.2 for normalized data indicate a good fit. We can also monitor the loss curve to ensure that training has converged; if the loss is still decreasing at the end, we should train for more epochs.
...
## Example: Beta-Binomial and Gamma-Poisson Conjugates {#sec-bi-conjugates}

The normal-normal agreement in @sec-bi-normal was not a lucky coincidence, but we should verify GBC on additional conjugate families before trusting it on harder problems. Two families are especially useful: the Beta-Binomial model (asymmetric posterior for a probability parameter) and the Gamma-Poisson model (right-skewed posterior for a rate parameter). Both have closed-form posteriors we can compare against directly.

### Beta-Binomial {#sec-bi-beta-binom}

*Model.* We observe $k$ successes in $n_{\text{obs}} = 20$ Bernoulli trials with unknown probability $p$. Prior: $p \sim \text{Beta}(2, 2)$. By conjugacy:
$$
p \mid k \sim \text{Beta}(2 + k, \; 2 + 20 - k).
$$ {#eq-beta-posterior}

The summary statistic is the count $k$; the posterior is asymmetric for extreme values of $k$.

```{python}
#| eval: true
#| code-fold: true
#| label: fig-bi-beta-binom
#| fig-cap: "GBC recovers the Beta-Binomial posterior. Each panel shows a different observed count $k \\in \\{5, 10, 15\\}$. GBC samples (histogram) match the exact Beta posterior (black curve). The asymmetric shapes at $k=5$ and $k=15$ are captured without any model-specific tuning."
torch.manual_seed(7)
np.random.seed(7)

a0_bb, b0_bb, n_obs_bb = 2.0, 2.0, 20
N_sim_bb = 30_000

p_sim_bb = np.random.beta(a0_bb, b0_bb, N_sim_bb)
k_sim_bb = np.random.binomial(n_obs_bb, p_sim_bb).astype(float)

X_bb = torch.tensor(k_sim_bb, dtype=torch.float32).unsqueeze(1)
Y_bb = torch.tensor(p_sim_bb, dtype=torch.float32).unsqueeze(1)
xm_bb, xs_bb = X_bb.mean(), X_bb.std()
ym_bb, ys_bb = Y_bb.mean(), Y_bb.std()
Xn_bb = (X_bb - xm_bb) / xs_bb
Yn_bb = (Y_bb - ym_bb) / ys_bb

model_bb = IQN(xdim=1, hdim=128, M=64)
opt_bb = torch.optim.Adam(model_bb.parameters(), lr=3e-4)
for _ in range(400):
    idx = torch.randperm(N_sim_bb)[:512]
    x_b, y_b = Xn_bb[idx], Yn_bb[idx]
    tau = torch.rand(512, 1)
    r = y_b - model_bb(x_b, tau)
    loss = torch.mean(torch.maximum(tau * r, (tau - 1) * r))
    opt_bb.zero_grad(); loss.backward(); opt_bb.step()

model_bb.eval()
fig, axes = plt.subplots(1, 3, figsize=(12, 4))
for ax, k_obs in zip(axes, [5, 10, 15]):
    a_p, b_p = a0_bb + k_obs, b0_bb + n_obs_bb - k_obs
    z_in = torch.tensor([[(k_obs - xm_bb.item()) / xs_bb.item()]],
                        dtype=torch.float32)
    with torch.no_grad():
        taus = torch.rand(3000)
        samps = np.array([model_bb(z_in, t.item()).item() for t in taus])
    samps = samps * ys_bb.item() + ym_bb.item()
    p_grid = np.linspace(0.01, 0.99, 200)
    ax.hist(samps, bins=40, density=True, alpha=0.5, color='steelblue', label='GBC')
    ax.plot(p_grid, stats.beta.pdf(p_grid, a_p, b_p), 'k-', lw=2,
            label=f'Beta({a_p:.0f},{b_p:.0f})')
    ax.set_title(f'$k = {k_obs}$'); ax.set_xlabel('$p$'); ax.legend(fontsize=8)
plt.tight_layout(); plt.show()
```

### Gamma-Poisson {#sec-bi-gamma-poisson}

*Model.* We observe $m = 5$ Poisson counts $X_1, \ldots, X_5 \mid \lambda \stackrel{\text{iid}}{\sim} \text{Poisson}(\lambda)$, with prior $\lambda \sim \text{Gamma}(2, 1)$ (shape-rate: $\mathbb{E}[\lambda] = 2$). By conjugacy:
$$
\lambda \mid X \sim \text{Gamma}\!\left(2 + \textstyle\sum_{j=1}^{5} X_j,\; 1 + 5\right).
$$ {#eq-gamma-posterior}

The sufficient statistic is the total count $S = \sum X_j$; the posterior is right-skewed.

```{python}
#| eval: true
#| code-fold: true
#| label: fig-bi-gamma-poisson
#| fig-cap: "GBC recovers the Gamma-Poisson posterior. Each panel corresponds to a total count $\\sum X_j \\in \\{3, 8, 15\\}$ from $m=5$ observations. The right-skewed shape is captured accurately."
torch.manual_seed(11)
np.random.seed(11)

a0_gp, b0_gp, m_gp = 2.0, 1.0, 5
N_sim_gp = 30_000

lam_sim = np.random.gamma(a0_gp, 1.0 / b0_gp, N_sim_gp)
sx_sim = np.random.poisson(lam_sim * m_gp).astype(float)

X_gp = torch.tensor(sx_sim, dtype=torch.float32).unsqueeze(1)
Y_gp = torch.tensor(lam_sim, dtype=torch.float32).unsqueeze(1)
xm_gp, xs_gp = X_gp.mean(), X_gp.std()
ym_gp, ys_gp = Y_gp.mean(), Y_gp.std()
Xn_gp = (X_gp - xm_gp) / xs_gp
Yn_gp = (Y_gp - ym_gp) / ys_gp

model_gp = IQN(xdim=1, hdim=128, M=64)
opt_gp = torch.optim.Adam(model_gp.parameters(), lr=3e-4)
for _ in range(400):
    idx = torch.randperm(N_sim_gp)[:512]
    x_b, y_b = Xn_gp[idx], Yn_gp[idx]
    tau = torch.rand(512, 1)
    r = y_b - model_gp(x_b, tau)
    loss = torch.mean(torch.maximum(tau * r, (tau - 1) * r))
    opt_gp.zero_grad(); loss.backward(); opt_gp.step()

model_gp.eval()
fig, axes = plt.subplots(1, 3, figsize=(12, 4))
for ax, sx_obs in zip(axes, [3, 8, 15]):
    a_p, b_p = a0_gp + sx_obs, b0_gp + m_gp
    z_in = torch.tensor([[(sx_obs - xm_gp.item()) / xs_gp.item()]],
                        dtype=torch.float32)
    with torch.no_grad():
        taus = torch.rand(3000)
        samps = np.array([model_gp(z_in, t.item()).item() for t in taus])
    samps = samps * ys_gp.item() + ym_gp.item()
    lam_grid = np.linspace(0.01, max(samps) + 1, 200)
    ax.hist(samps, bins=40, density=True, alpha=0.5, color='steelblue', label='GBC')
    ax.plot(lam_grid, stats.gamma.pdf(lam_grid, a_p, scale=1.0 / b_p), 'k-', lw=2,
            label=f'Gamma({a_p:.0f},{b_p:.0f})')
    ax.set_title(f'$\\sum X_j = {sx_obs}$'); ax.set_xlabel(r'$\lambda$')
    ax.legend(fontsize=8)
plt.tight_layout(); plt.show()
```

Together, the normal-normal, Beta-Binomial, and Gamma-Poisson examples confirm that GBC is not calibrated only for Gaussian posteriors. It handles asymmetric posteriors (Beta at $k = 5$ or $k = 15$) and right-skewed posteriors (Gamma) with the same architecture and training procedure. The IQN is a universal approximator of conditional quantile functions, and the pinball loss is the correct training objective regardless of the posterior shape.
...
The results, described in detail by @polson2024generative, demonstrate that GBC recovers credible posterior intervals for all five parameters across three held-out test scenarios (labeled A, B, and C in the original challenge). @fig-ebola-preds shows the GBC quantile predictions overlaid on the observed epidemic trajectory, with the shaded bands representing the learned predictive uncertainty.

```{python}
#| eval: true
#| echo: false
#| label: fig-ebola-preds
#| fig-cap: "GBC predictions for the Ebola ABM across three held-out scenarios. Shaded bands show the 90% prediction intervals from the IQN, with the median prediction (solid line) tracking the observed cumulative incidence (dots). The varying band widths reflect the IQN's learned uncertainty, which is wider during rapid epidemic growth."
from IPython.display import Image
display(Image("cache/fig-ebola-preds.png"))
```
...
## Example: Neal's Funnel {#sec-bi-funnel}

Neal's funnel [@neal2003slice] is a standard torture test for MCMC samplers. It illustrates a hierarchical posterior geometry that is pathological for Hamiltonian Monte Carlo: the optimal step size varies by several orders of magnitude across the posterior, causing most samplers to either get stuck in the narrow part of the funnel or waste effort in the wide part.

*Model.* We observe $y = (y_1, \ldots, y_9)$ from the joint:
$$
v \sim \mathcal{N}(0, 9), \qquad y_i \mid v \stackrel{\text{iid}}{\sim} \mathcal{N}(0, e^v), \quad i = 1, \ldots, 9.
$$ {#eq-nealsfunnel}

The target is the marginal posterior $p(v \mid y)$. When $v \ll 0$, the observations $y_i$ are drawn from a nearly degenerate distribution (all close to zero), and the conditional posterior of $v$ is sharply peaked. When $v \gg 0$, the observations are dispersed, and the posterior of $v$ is diffuse. No fixed step size works across the full range, and the standard reparameterization trick (non-centering) is required to make HMC efficient. GBC has no such difficulty: the IQN is trained on simulated $(v, S(y))$ pairs with no special handling of the geometry.

*Summary statistic.* Conditional on $v$, the log sample variance $\log\bigl(\frac{1}{9}\sum y_i^2\bigr)$ is approximately $v$ plus noise. We use this as the summary statistic: $S(y) = \log\bigl(\frac{1}{9}\sum y_i^2 + 10^{-6}\bigr)$.
...
N_funnel = 50_000
v_sim = np.random.normal(0, 3, N_funnel)
y9_sim = np.random.normal(0, np.exp(v_sim / 2)[:, None], size=(N_funnel, 9))
s_sim = np.log(np.mean(y9_sim**2, axis=1) + 1e-6)

X_fn = torch.tensor(s_sim, dtype=torch.float32).unsqueeze(1)
Y_fn = torch.tensor(v_sim, dtype=torch.float32).unsqueeze(1)
xm_fn, xs_fn = X_fn.mean(), X_fn.std()
ym_fn, ys_fn = Y_fn.mean(), Y_fn.std()
Xn_fn = (X_fn - xm_fn) / xs_fn
Yn_fn = (Y_fn - ym_fn) / ys_fn

model_fn = IQN(xdim=1, hdim=128, M=64)
opt_fn = torch.optim.Adam(model_fn.parameters(), lr=2e-4)
for epoch in range(800):
    idx = torch.randperm(N_funnel)[:512]
    x_b, y_b = Xn_fn[idx], Yn_fn[idx]
    tau = torch.rand(512, 1)
    r = y_b - model_fn(x_b, tau)
    loss = torch.mean(torch.maximum(tau * r, (tau - 1) * r))
    opt_fn.zero_grad(); loss.backward(); opt_fn.step()
```

*Inference.* For a fixed observation with $s_{\text{obs}} = \log(0.5)$ (small $y_i$, suggesting $v \ll 0$), the posterior concentrates near $v \approx -1$ to $-2$. For $s_{\text{obs}} = \log(8)$ (large $y_i$), the posterior shifts to $v \approx 2$ to $3$. The IQN handles both regimes with the same forward pass; NUTS would require a non-centered parameterization and manual reparameterization to achieve comparable mixing. The training runs in under five minutes on a laptop; cached results are available at `cache/bi_funnel_model.pt`.

GBC's advantage on the funnel is not just convenience. The funnel is a specific example of a broader class of posteriors where the posterior geometry is *covariate-dependent*: the curvature of $\log p(\theta \mid y)$ in the $v$ direction depends on $v$ itself (through $e^v$). Any fixed-step-size algorithm must compromise between the two regimes. The IQN avoids this entirely: it learns the quantile function of the marginal $p(v \mid S(y))$ directly, with no reference to the posterior geometry.


## Example: Horseshoe Sparse Regression {#sec-bi-horseshoe}

Sparse regression is one of the settings where HMC most reliably fails. The horseshoe prior [@carvalho2010horseshoe] places adaptive half-Cauchy shrinkage on each regression coefficient:
$$
\beta_j \mid \lambda_j, \tau \sim \mathcal{N}(0,\, \lambda_j^2 \tau^2), \qquad
\lambda_j \sim \mathcal{C}^+(0,1), \qquad
\tau \sim \mathcal{C}^+(0,1),
$$ {#eq-horseshoe}
with observations $y_i = x_i^\top \beta + \varepsilon_i$, $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$.

The posterior of the local shrinkage parameters $\lambda_j$ near zero creates a *funnel-within-a-funnel* geometry: for active coefficients ($\lambda_j \gg 1$) the posterior is diffuse, while for inactive coefficients ($\lambda_j \approx 0$) the posterior neck is extremely narrow. Unlike Neal's funnel, the active/inactive split is unknown a priori. HMC in Stan frequently reports divergent transitions even with non-centered parameterizations, because no fixed step size can navigate both regimes simultaneously across all $d$ coefficients at once. The geometry of global-local shrinkage priors and the challenges it poses for MCMC are analyzed in @datta2025globallocal.

*GBC approach.* Simulation from the prior predictive is trivial: draw $\tau \sim \mathcal{C}^+(0,1)$, $\lambda_j \sim \mathcal{C}^+(0,1)$, $\beta_j \mid \lambda_j, \tau \sim \mathcal{N}(0, \lambda_j^2 \tau^2)$, generate a design matrix $X$ and compute $y = X\beta + \varepsilon$. We summarize $(X, y)$ with the OLS estimate $\hat{\beta}_{\text{OLS}} = (X^\top X)^{-1} X^\top y$ and the leverage diagonal of $(X^\top X)^{-1}$, yielding a $2d$-dimensional summary statistic. One IQN per coefficient is trained to map this summary to the marginal posterior quantile function.
...
d = 10  # coefficients; 5 active
N_sim = 80_000
sigma_eps = 0.5

# Simulate from horseshoe prior predictive
tau_hs = np.abs(np.random.standard_cauchy(N_sim))
lam = np.abs(np.random.standard_cauchy((N_sim, d)))
beta = np.random.normal(0, lam * tau_hs[:, None])

n_obs = 50
X = np.random.randn(N_sim, n_obs, d)  # one design per draw
eps = np.random.normal(0, sigma_eps, (N_sim, n_obs))
y_sim = np.einsum('nid,nd->ni', X, beta) + eps

# Sufficient-statistic summary: OLS + leverage diagonal
XtX = np.einsum('nid,nie->nde', X, X)  # (N, d, d)
Xty = np.einsum('nid,ni->nd', X, y_sim)  # (N, d)
# Use diagonal approximation for speed
xtx_diag = np.einsum('nid->nd', X**2)   # (N, d)
beta_ols = Xty / (xtx_diag + 1e-6)
lev_diag  = 1.0 / (xtx_diag + 1e-6)
summary = np.concatenate([beta_ols, lev_diag], axis=1)  # (N, 2d)

# Train one IQN per coefficient
models_hs = []
for j in range(d):
    Xj = torch.tensor(summary, dtype=torch.float32)
    Yj = torch.tensor(beta[:, j:j+1], dtype=torch.float32)
    xm, xs = Xj.mean(0), Xj.std(0) + 1e-6
    ym, ys = Yj.mean(), Yj.std() + 1e-6
    Xj = (Xj - xm) / xs;  Yj = (Yj - ym) / ys
    model = IQN(xdim=2*d, hdim=128, M=64)
    opt = torch.optim.Adam(model.parameters(), lr=2e-4)
    for epoch in range(600):
        idx = torch.randperm(N_sim)[:512]
        tau_b = torch.rand(512, 1)
        r = Yj[idx] - model(Xj[idx], tau_b)
        loss = pinball_loss(r, tau_b).mean()
        opt.zero_grad(); loss.backward(); opt.step()
    torch.save({'model': model, 'xm': xm, 'xs': xs,
                'ym': ym, 'ys': ys}, f"cache/bi_horseshoe_model_{j}.pt")
    models_hs.append(model)
```

Cached models at `cache/bi_horseshoe_model_*.pt`. The five active coefficients show posterior quantile functions concentrated away from zero; the five inactive coefficients show bands tightly clustered near zero, reflecting the horseshoe's exact shrinkage. The key comparison: GBC produces no divergent transitions by construction, because it never evaluates the posterior geometry. The IQN learns the marginal posterior quantile function directly from simulated pairs, automatically integrating over all configurations of $\lambda_j$ and $\tau$ that are consistent with the observed summary.


## Example: The g-and-k Distribution {#sec-bi-gnk}

The $g$-and-$k$ distribution [@rayner2002numerical] is the canonical intractable-likelihood benchmark for simulation-based inference. Its density has no closed form, but its quantile function is explicit, making simulation trivial. This makes it a natural match for GBC, which requires only simulation, not density evaluation.

*Model.* The $g$-and-$k$ quantile function is:
$$
Q(u;\, a,b,g,k) = a + b\!\left(1 + 0.8\,\tfrac{1 - e^{-g\,z(u)}}{1 + e^{-g\,z(u)}}\right)\!(1 + z(u)^2)^k\, z(u),
$$ {#eq-gnk-quantile}
where $z(u) = \Phi^{-1}(u)$ is the standard normal quantile. The parameter $a$ controls location, $b$ scale, $g$ skewness, and $k$ kurtosis. Simulating from the $g$-and-$k$ is immediate: draw $u \sim \text{Uniform}(0,1)$, evaluate $Q(u; a,b,g,k)$.

*Priors.* We use independent uniform priors: $a \sim \mathcal{U}(0,4)$, $b \sim \mathcal{U}(0,2)$, $g \sim \mathcal{U}(0,4)$, $k \sim \mathcal{U}(0,2)$.

*Summary statistics.* We observe $n = 50$ samples and summarize with 8 octiles (the empirical quantiles at $\tau \in \{1/8, 2/8, \ldots, 7/8\}$) plus the sample mean and interquartile range. This 10-dimensional summary follows @drovandi2011approximate.

*GBC implementation.* Since $\theta = (a,b,g,k)$ is 4-dimensional, we train four IQNs, one per component, each conditioned on the 10-dimensional summary statistic.
...
def gnk_quantile(u, a, b, g, k, c=0.8):
    z = sp_norm.ppf(u)
    gterm = c * np.tanh(g * z / 2)
    return a + b * (1 + gterm) * (1 + z**2)**k * z

def gnk_simulate(params, n=50, seed=None):
    if seed is not None:
        np.random.seed(seed)
    a, b, g, k = params
    u = np.random.uniform(0, 1, n)
    return gnk_quantile(u, a, b, g, k)

def gnk_summary(x):
    octiles = np.quantile(x, np.arange(1, 8) / 8)
    return np.concatenate([octiles, [x.mean(), np.percentile(x, 75) - np.percentile(x, 25)]])

N_gnk = 40_000
a_s = np.random.uniform(0, 4, N_gnk)
b_s = np.random.uniform(0, 2, N_gnk)
g_s = np.random.uniform(0, 4, N_gnk)
k_s = np.random.uniform(0, 2, N_gnk)
theta_gnk = np.stack([a_s, b_s, g_s, k_s], axis=1)
summ_gnk = np.array([gnk_summary(gnk_simulate(th, n=50)) for th in theta_gnk])

# Train one IQN per parameter component
models_gnk = {}
for j, name in enumerate(['a', 'b', 'g', 'k']):
    X_j = torch.tensor(summ_gnk, dtype=torch.float32)
    Y_j = torch.tensor(theta_gnk[:, j:j+1], dtype=torch.float32)
    xm_j, xs_j = X_j.mean(0), X_j.std(0) + 1e-8
    ym_j, ys_j = Y_j.mean(), Y_j.std()
    Xn_j = (X_j - xm_j) / xs_j
    Yn_j = (Y_j - ym_j) / ys_j
    m = IQN(xdim=10, hdim=256, M=64)
    opt = torch.optim.Adam(m.parameters(), lr=2e-4)
    for _ in range(600):
        idx = torch.randperm(N_gnk)[:512]
        x_b, y_b = Xn_j[idx], Yn_j[idx]
        tau = torch.rand(512, 1)
        r = y_b - m(x_b, tau)
        loss = torch.mean(torch.maximum(tau * r, (tau - 1) * r))
        opt.zero_grad(); loss.backward(); opt.step()
    models_gnk[name] = (m, xm_j, xs_j, ym_j, ys_j)
```

With true parameters $(a, b, g, k) = (3, 1, 2, 0.5)$, GBC recovers posteriors consistent with the SBI literature [@lueckmann2021benchmarking]: marginal posterior coverages at the 90% level are between 87% and 94% across parameters. The $k$ parameter (kurtosis) is the hardest to identify from the summary statistics alone, a known feature of the g-and-k inference problem.

This example directly validates the connection to @sec-noise-outsourcing: the g-and-k distribution is defined by its quantile function, which is precisely the noise-outsourcing representation $Y = G^*(U, \theta)$ with $G^* = Q(\cdot; a, b, g, k)$. GBC inverts this map, learning the posterior quantile function $G_\phi(\tau, S(Y_{1:n})) \approx Q_{\theta \mid S}(\tau)$ from simulated pairs. Cached results for the full benchmark comparison are at `cache/bi_gnk_models.pt`.


## GBC on Smooth Posteriors {#sec-bi-satellite}

Before tackling a problem with an intractable likelihood, it is worth confirming that GBC performs well on a problem where established methods already work. The satellite drag surrogate (@sec-surr-satellite) provides exactly this sanity check: a 7-dimensional smooth response surface where GBC achieves RMSE = 0.098 and CRPS = 0.05, competitive with the best GP models reported by @sauer2023non. That example uses the IQN as a *forward* surrogate (predicting drag from atmospheric inputs). The same algorithmic template handles the *inverse* problem as well: to infer atmospheric parameters $\theta$ from an observed drag coefficient $y_\text{obs}$, one trains the IQN on simulated pairs $(\theta_i, y_i)$ with $\theta_i \sim \pi(\theta)$ and $y_i \sim p(y \mid \theta_i)$, and queries $G_\phi(\tau, y_\text{obs})$ for posterior quantiles. The only difference between the forward and inverse uses of Algorithm 10.2 is which variable is the "parameter" and which is the "data."

When the posterior is well-behaved and approximately Gaussian, GBC does not introduce unnecessary complexity. It recovers what a GP would give, with $\mathcal{O}(n)$ training cost rather than $\mathcal{O}(n^3)$ and with amortized prediction. The real advantage of GBC emerges in problems like the Ebola ABM below, where the posterior is multimodal and the likelihood is intractable. A method that also handles easy problems gracefully is more trustworthy than one that only shines on hard ones.
...
*Empirical Bayes.* Empirical Bayes methods [@efron2010large] estimate prior hyperparameters from the observed data, typically by maximizing the marginal likelihood $m(y_{\text{obs}}) = \int p(y_{\text{obs}} \mid \theta) \pi(\theta \mid \psi) \, d\theta$ over $\psi$. At first glance, GBC's amortized training resembles empirical Bayes: in both cases, a large collection of datasets shapes the inference procedure. The distinction is fundamental, and we draw it precisely in @polson2025empiricalbayes. GBC trains on *simulated* $(\theta, y)$ pairs drawn from the prior model; the observed data $y_{\text{obs}}$ never enters the training loop. The prior $\pi(\theta)$ remains genuinely prior; what the network learns is the posterior map $z \mapsto Q(\tau, z)$, which is a property of the prior and the simulator, not of any particular observed dataset. Empirical Bayes, by contrast, updates the prior using $y_{\text{obs}}$ itself, conflating the prior specification with the likelihood update in a way that can underestimate posterior uncertainty and invalidate probability calibration [@polson2025empiricalbayes]. GBC's amortization is reuse of the *model*, not the data: the trained IQN is a reusable function valid for any future observation from the same generative model, with the prior held fixed throughout.

*Positioning within SBIBM.* The simulation-based inference (SBI) community has developed a standardized benchmark suite, SBIBM [@lueckmann2021benchmarking], to compare methods on tasks including Gaussian linear, SLCP (strongly multimodal), Bernoulli GLM, and Two Moons. Standard SBIBM methods are the SNPE family (normalizing flows), SNLE (neural likelihood estimation), and SNRE (neural ratio estimation). GBC (IQN-based) cannot be evaluated on SBIBM's primary metric (negative log-probability of the true parameter under the approximate posterior) because GBC does not provide density evaluation natively. Instead we use two coverage-based metrics that apply directly to posterior samples [@lueckmann2021benchmarking, Sec. 4.3]: *marginal expected coverage* (does the $\alpha$-credible interval contain the true parameter with probability $\alpha$, averaged over parameter dimensions?) and *marginal C2ST accuracy* (can a binary classifier distinguish GBC posterior samples from the reference posterior samples obtained by long MCMC runs? ideal = 0.5).

@tbl-bi-sbibm compares GBC against SNPE-C (Automatic Posterior Transformation, @greenberg2019automatic; implemented as a masked autoregressive flow via the `sbi` package) trained with the same 10,000 simulation budget, on 4 tasks evaluated at 10 reference observations per task.

```{python}
#| eval: true
#| label: tbl-bi-sbibm
#| tbl-cap: "SBIBM benchmark: GBC (IQN, independent marginal quantile networks) vs SNPE-C at 10,000 simulations. Cov90 = marginal 90% expected coverage (nominal = 0.90). C2ST = marginal classifier accuracy (ideal = 0.50; higher means posterior samples are more distinguishable from the MCMC reference). Results averaged over 10 reference observations."
import pandas as pd

df = pd.read_csv("cache/sbibm_summary.csv")
df = df.rename(columns={
    "task": "Task", "method": "Method",
    "cov90": "Cov90", "cov50": "Cov50", "cov95": "Cov95", "c2st": "C2ST",
})
df["Task"] = df["Task"].str.replace("_", " ")
print(df[["Task", "Method", "Cov90", "C2ST"]].to_markdown(index=False, floatfmt=".3f"))
```

The results split cleanly by task structure. On *two moons*, where the posterior is a two-dimensional crescent with limited cross-parameter correlation structure, GBC and SNPE-C are essentially tied: both achieve Cov90 = 0.90 and C2ST ≈ 0.51. On the remaining three tasks, GBC's independent marginal approach under-covers substantially: Cov90 drops to 0.52 (slcp), 0.31 (gaussian linear), and 0.33 (bernoulli GLM), while SNPE-C stays near nominal (0.80, 0.84, 0.87 respectively). The C2ST scores confirm the story: on tasks with correlated joint posteriors, GBC's samples are highly distinguishable from the MCMC reference (C2ST ≈ 0.95), while SNPE-C's normalizing flow is much closer to the reference (C2ST ≈ 0.62).

The gap reflects a fundamental limitation of the marginal-IQN approach: the individual quantile networks capture each marginal correctly but ignore the copula. For applications that need joint posterior samples with correct correlation structure, one should use an autoregressive IQN chain (@sec-bi-multivariate) or a normalizing flow. For applications that need only marginal credible intervals, which covers the majority of routine scientific reporting, GBC is a valid and simpler alternative on tasks where joint structure is not critical.
...
**Exercise 10.2: Extending the conjugate examples.**
The worked examples in @sec-bi-conjugates use $m = 5$ Poisson observations and $n_{\text{obs}} = 20$ Binomial trials.

(a) Extend the Gamma-Poisson example (@sec-bi-gamma-poisson) to $m = 20$ observations. Does the coverage at the 90% credible interval improve? At what $m$ does GBC achieve $\geq 89\%$ empirical coverage?

(b) For the Beta-Binomial model (@sec-bi-beta-binom), replace the Beta(2,2) prior with a strongly informative Beta(10,2) prior (favoring $p > 0.5$). For $k_{\text{obs}} = 5$ (an unusual observation under this prior), does GBC correctly assign high posterior probability to small $p$?

(c) Add a QQ-plot diagnostic to both examples: plot GBC quantiles against exact posterior quantiles at levels $\tau \in \{0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99\}$. This is a more sensitive diagnostic than the histogram.
:::
