...
A reader who has followed the book sequentially will recognize a recurring pattern: we have been building the GBC inference engine one piece at a time, starting from the abstract theory of quantile functions, through the constructive existence result of noise outsourcing, to the concrete neural network architecture of the IQN. This chapter assembles these pieces into a complete inference pipeline and demonstrates that it works --- first on problems where we can verify against exact posteriors, then on problems where no alternative exists.

The structure of this chapter also mirrors the structure of a typical applied Bayesian workflow. We begin with the simplest possible model (normal-normal) to build intuition and verify the methodology. We then tackle a harder model (bimodal posterior) that tests the method's ability to handle complex posterior shapes. We consider the multivariate case, which is necessary for realistic applications. We apply the method to two real-world problems: one where the likelihood is intractable (Ebola ABM) and one where the likelihood is tractable but the GBC approach offers computational advantages (satellite drag). We position the method relative to the standard toolbox and honestly assess its limitations. This progression from simple to complex, from validation to application, is the template we recommend for any practitioner adopting GBC for a new problem domain.


## The Bayesian Inference Problem {#sec-bi-problem}

Before diving into the GBC algorithm, we need to be precise about what we mean by "Bayesian inference" and what makes it computationally hard. This section states the problem; the rest of the chapter solves it.
...
The standard solution, developed over the past four decades, is Markov chain Monte Carlo (MCMC): construct a Markov chain whose stationary distribution is $\pi(\theta \mid y)$, run it for $T$ iterations, and use the samples $\theta^{(1)}, \ldots, \theta^{(T)}$ as a Monte Carlo approximation to the posterior. The genius of MCMC is that it *avoids computing the marginal likelihood entirely*: the Metropolis-Hastings acceptance ratio $p(y \mid \theta') \pi(\theta') / [p(y \mid \theta) \pi(\theta)]$ cancels the denominator, so we only need to evaluate the unnormalized posterior $p(y \mid \theta) \pi(\theta)$.

MCMC is a profound contribution to statistics --- arguably the most important computational development in the field since the bootstrap. The Metropolis algorithm [@metropolis1953equation], the Gibbs sampler [@geman1984stochastic; @gelfand1990sampling], and Hamiltonian Monte Carlo [@neal2011mcmc] are workhorses of modern Bayesian analysis, implemented in powerful software packages like Stan [@carpenter2017stan], JAGS, and NumPyro. But MCMC has well-known limitations that motivate the search for alternatives. Convergence is uncertain: how long we must run the chain before trusting the samples depends on the geometry of the posterior, and no general-purpose diagnostic guarantees convergence --- in practice we run multiple chains and check for agreement, but this is heuristic. The $\hat{R}$ diagnostic of @gelman1992inference is the most widely used convergence check: it compares the between-chain variance to the within-chain variance, and values near 1 suggest (but do not guarantee) convergence. Even with $\hat{R} < 1.01$ for all parameters, there is no guarantee that the chain has discovered all modes of a multimodal posterior.

Mixing can fail in high-dimensional or multimodal posteriors, where chains get trapped in local modes; sophisticated algorithms such as Hamiltonian Monte Carlo and parallel tempering help, but the problem is fundamental. For the bimodal example we will encounter in @sec-bi-bimodal, a standard Metropolis chain might spend 10,000 iterations in one mode before (possibly) jumping to the other. The result is a posterior approximation that severely underrepresents one mode and overrepresents the other.

Cost scales with the number of datasets: MCMC must be rerun from scratch for each new observation, so fitting the same model to 10,000 sensors requires 10,000 separate chains, each costing $O(Tn)$ where $T$ is the chain length and $n$ is the dataset size. Finally, MCMC requires evaluating $p(y \mid \theta)$ at each iteration, which rules it out entirely for simulator-based models where we can *generate* data from the model but cannot write down a closed-form likelihood. The question is not whether MCMC is correct (it is, asymptotically), but whether there is a more efficient computational route to the same target.


## ABC: The First Likelihood-Free Method {#sec-bi-abc}

The limitations of MCMC motivate the search for alternatives. But the most radical limitation is not about efficiency or convergence --- it is about *scope*. MCMC requires evaluating the likelihood $p(y \mid \theta)$ at every proposed parameter value. For many of the most interesting scientific models --- agent-based simulations, complex physical codes, stochastic network models --- we can *run the model forward* (given $\theta$, generate $y$) but we cannot *write down the likelihood* (given $\theta$ and $y$, compute $p(y \mid \theta)$). These are called *implicit* or *simulator-based* models, and they are ubiquitous in the sciences. For such models, MCMC is not merely inefficient --- it is inapplicable.

The first breakthrough in likelihood-free inference came from the genetics community. @rubin1984bayesianly observed that one could interpret Bayesian inference purely through simulation: draw $\theta$ from the prior, simulate data $y^{\text{sim}}$ from the model, and keep those $\theta$ values for which $y^{\text{sim}}$ matches $y_{\text{obs}}$. The resulting accepted samples are exact draws from the posterior --- if we could achieve exact matching.
...
Having motivated the move from ABC to GBC, we now state the core algorithm precisely. The elegance of the GBC approach lies in its simplicity: it reduces Bayesian inference to a supervised learning problem. We simulate a training set from the joint distribution of parameters and data, then train a neural network to predict parameters given data. The key innovation is that the network learns the *conditional quantile function* rather than the conditional mean, which allows it to represent the full posterior distribution rather than a point estimate.
...
$$
where $\rho_\tau(u) = \max(\tau u, (\tau - 1)u)$ is the pinball loss and $\tau \sim \text{Uniform}(0, 1)$.
...
This result has a deep connection to the theory of Bayes estimators. The function $S(y) = \mathbb{E}[\theta \mid y]$ is the *Bayes estimator* under squared error loss. It maps the data $y$ to the posterior mean, which is the point estimate that minimizes the expected squared distance to the true parameter. But we are not interested in point estimation --- we want the full posterior. The insight is that this same function $S$ also serves as the optimal input to the IQN, because it compresses the data into the most informative finite-dimensional summary. Any other summary of the same dimension would lose information relative to the posterior mean.
...
We begin with the simplest Bayesian model: the conjugate normal-normal. This is the "hydrogen atom" of Bayesian inference --- simple enough to solve exactly, yet rich enough to illustrate the key ideas. The purpose of this example is not to argue that GBC should replace conjugate analysis for normal models --- it should not --- but to provide a rigorous validation. If GBC cannot recover the exact posterior for a model where the exact posterior is known in closed form, we would have no reason to trust it on harder problems where the posterior is unknown. This example establishes the baseline.
...
*Wang distortion interpretation.* The prior-to-posterior update has an elegant interpretation in quantile space. The prior CDF is $F_\theta(t) = \Phi\!\left(\frac{t - \mu_0}{\sigma_0}\right)$ and the posterior CDF is $F_{\theta \mid y}(t) = \Phi\!\left(\frac{t - \mu_n}{\sigma_n}\right)$. These are related by a *Wang distortion* [@wang2000class]:
$$
...
The practical implications of amortization are significant. Consider three concrete scenarios where amortization matters. In *sensor networks*, an environmental monitoring system might deploy 10,000 sensors, each producing a time series that must be analyzed with the same Bayesian model. With MCMC, each sensor requires a separate chain (potentially minutes to hours); with GBC, all 10,000 sensors are processed in seconds. In *clinical trials*, a pharmaceutical company may run a Bayesian adaptive design across hundreds of trial sites, updating the posterior for the treatment effect at each site as new patient data arrive. GBC makes these updates instantaneous. In *financial modeling*, a bank might apply the same risk model to millions of accounts; amortized GBC turns a computation that would take days with MCMC into one that takes minutes.

Beyond these production scenarios, amortization enables interactive posterior exploration. An analyst can adjust the observed data --- "what if we had seen $\bar{y} = 2$ instead of $\bar{y} = 1.5$?" --- and instantly see how the posterior changes. This kind of real-time sensitivity analysis is impossible with MCMC, where each posterior requires a fresh chain. Finally, simulation-based calibration [@lueckmann2021benchmarking] requires running inference on hundreds of simulated datasets and checking coverage; with GBC this is trivial, while with MCMC it is a major computational undertaking.
...
The classical Bayesian toolbox covers a wide range of computational strategies, and it is instructive to compare each with its GBC counterpart. We discuss the five most important comparisons in turn, moving from the most common method (MCMC) to the most specialized (importance sampling).
...
The foundation established in this chapter --- the GBC
algorithm, the noise outsourcing guarantee, the amortization
principle, and the honest assessment of when GBC excels and
when it does not --- will be the constant reference point
as we move into these applications. The reader should feel
confident that the method is well-grounded theoretically,
well-validated empirically, and honestly positioned relative
to alternatives.

To summarize the chapter's key messages, we highlight five properties
that make GBC a distinctive approach to Bayesian computation.
...
This is not merely an asymptotic guarantee --- it tells us
*what function* the network is converging to (the posterior
